...
namespace petoiAiVisionPy {
    ...
    const IMP = `from petoi_ai import ai`;
    ...
    //% PROMPT.shadow="string" PROMPT.defl="Guarda l'immagine e scegli un'azione. Rispondi in JSON {\"action\":\"...\",\"reason\":\"...\"}"
    ...
    //% block="acquisisci immagine dalla camera" blockType="command"
    ...
    //% block="chiedi all'AI sull'immagine, richiesta [USERTEXT]" blockType="reporter"
    //% USERTEXT.shadow="string" USERTEXT.defl="Cosa vedi? Scegli un'azione."
    export function askAboutImage(parameter: any, block: any) {
        Generator.addImport(IMP);
        Generator.addCode(`ai.ask_image(${parameter.USERTEXT.code})`);
    }

    //% block="chiedi all'AI (solo testo) [PROMPT]" blockType="reporter"
    ...
    //% block="metti il Petoi a riposo" blockType="command"
